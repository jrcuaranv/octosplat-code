...
scripts/octosplat.py, e.g. a "*_sgs_output*" folder containing
a "*_params.npz" file) on a held-out octosplat-format evaluation dataset folder
...
def load_data_sample(eval_data_dir, file_name, intrinsics, max_depth, device="cuda"):
...
def main():
    ...
    parser.add_argument("--model_dir", required=True,
                         help="SLAM output folder produced by octosplat.py, e.g. .../sgs_output_g1_row1")
    parser.add_argument("--eval_data_dir", required=True,
                         help="octosplat-format eval dataset folder (images/, depth/, semantics/, confidences/, poses/, intrinsics.txt)")
    ...
    parser.add_argument("--confidence_threshold", type=float, default=0.4, help="Minimum confidence value for a pixel to be considered valid in evaluation metrics")
    ...
    with torch.no_grad():
        for idx, file_name in enumerate(tqdm(file_names, desc="Evaluating")):
            ...
            psnr, ssim, lpips_score, rmse, depth_l1, miou = eval_single_frame(
                ...
                rendered_color, rendered_depth, rendered_semantics, args.confidence_threshold, args.max_depth, device=args.device)
